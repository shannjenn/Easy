 ...
 * 时间：2017/10/31.
 */
abstract class EasyTabBarTopScrollImp extends HorizontalScrollView {
...
//    private float mIndicatorMarginTop;
    ...
    public EasyTabBarTopScrollImp(Context context) {
        this(context, null, 0);
    }

    public EasyTabBarTopScrollImp(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
        this.mContext = context;
        initAttrs(attrs);
    }

    public EasyTabBarTopScrollImp(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        mIndicatorGravity = ta.getInt(R.styleable.EasyTabBarTopScroll_indicator_gravity, Gravity.BOTTOM);
    ...
    private void scrollTabIfEdge() {
        ...
        int x;
        if (isToRight) {
            x = (int) (left + newTv.getWidth() + scrollTv.getWidth() - mWidth);
        } else {
            x = (int) (left);
        }
        scrollTo(x, 0);
