...
public class EasyListView extends ListView {
    ...
    private EasyListViewHeader headerView;
    ...
    // footer
    private EasyListViewFooter footerView;
    // 是否可以加载更多
    ...
    public EasyListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initView(Context context) {
        ...
        headerView = new EasyListViewHeader(context);
        footerView = new EasyListViewFooter(context);
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (getFirstVisiblePosition() == 0) {
                    if (enableRefresh
                        ...
                        isRefreashing = true;
                        headerView.setState(EasyListViewHeader.STATE_REFRESHING);
    ...
    public void setPullLoadEnable(boolean enable) {
        ...
        if (!enableLoadMore) {
            ...
            isLoadingMore = false;
            footerView.show();
            footerView.setState(EasyListViewFooter.STATE_NORMAL);
    ...
    private void updateHeaderHeight(float delta) {
        ...
        if (enableRefresh && !isRefreashing) {
            if (headerView.getVisiableHeight() > headerHeight) {
                headerView.setState(EasyListViewHeader.STATE_READY);
            } else {
                headerView.setState(EasyListViewHeader.STATE_NORMAL);
            }
        }
    ...
    private void updateFooterHeight(float delta) {
        ...
        if (enableLoadMore && !isLoadingMore) {
            if (height > PULL_LOAD_MORE_DELTA) {
                footerView.setState(EasyListViewFooter.STATE_READY);
            } else {
                footerView.setState(EasyListViewFooter.STATE_NORMAL);
            }
        }
    ...
    private void startLoadMore() {
        isLoadingMore = true;
        footerView.setState(EasyListViewFooter.STATE_LOADING);
        if (mListViewListener != null) {
